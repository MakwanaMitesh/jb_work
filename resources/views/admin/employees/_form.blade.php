...
<div class="bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 rounded-2xl shadow-sm p-6 sm:p-8">
        ...
        <div>
            ...
            <select id="city_id" name="city_id" class="form-select w-full rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 focus:border-primary-500 focus:ring-primary-500/20 text-sm h-10 px-3 select2">
                ...
                @foreach ($cities as $city)
                    <option value="{{ $city->id }}" @selected(old('city_id', $employee?->city_id) == $city->id)>{{ $city->name }}</option>
                @endforeach
            </select>
    ...
    <div class="mt-6">
        ...
        <textarea id="address" name="address" rows="3" class="w-full rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 placeholder-slate-400 focus:border-primary-500 focus:ring-primary-500/20 text-sm px-3 py-2">{{ old('address', $employee?->address) }}</textarea>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mt-6">
        <div>
            ...
            <select id="role" name="role" class="form-select w-full rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 focus:border-primary-500 focus:ring-primary-500/20 text-sm h-10 px-3" @disabled(! auth()->user()->can('employees.assign_role'))>
    ...
    <div class="mt-6 flex flex-col sm:flex-row items-center gap-6 p-4 rounded-2xl bg-slate-50 dark:bg-slate-900/50 border border-slate-100 dark:border-slate-800/60">
        ...
        <div class="flex-1 space-y-2">
            ...
            <div class="flex items-center gap-3">
                ...
                <button type="button" id="remove_photo_btn" onclick="window.clearImage(this)" class="px-4 py-2 border border-red-200 dark:border-red-900/50 bg-red-50 dark:bg-red-950/10 text-red-600 hover:bg-red-100 dark:hover:bg-red-950/20 rounded-xl text-sm font-semibold transition shrink-0 {{ ($employee?->profilePhotoUrl()) ? '' : 'hidden' }}">
                    Remove
                </button>
            </div>
    ...
    <div class="mt-8 pt-8 border-t border-slate-100 dark:border-slate-800">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                <x-input-label for="insurance_policy_pdf" value="Insurance Policy Document (PDF/Image)" />
                ...
                <div class="flex items-center gap-3">
                    ...
                    @if ($employee?->insurance_policy_pdf_path)
                        <a href="{{ $employee->insurancePolicyPdfUrl() }}" target="_blank" class="download-link inline-flex items-center justify-center w-10 h-10 rounded-xl border border-slate-200 dark:border-slate-800 text-slate-500 hover:bg-slate-50 shrink-0" title="Download Document">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M3 16.5v2.25A2.25 2.25 0 005.25 21h13.5A2.25 2.25 0 0021 18.75V16.5M16.5 12L12 16.5m0 0L7.5 12m4.5 4.5V3"/></svg>
                        </a>
                    @endif
                    <button type="button" id="remove_insurance_pdf_btn" onclick="window.clearFormFile(this, 'remove_insurance_policy_pdf', '.insurance-preview-img', '.insurance-placeholder-icon')" class="px-4 py-2 border border-red-200 dark:border-red-900/50 bg-red-50 dark:bg-red-950/10 text-red-600 hover:bg-red-100 dark:hover:bg-red-950/20 rounded-xl text-sm font-semibold transition shrink-0 {{ ($employee?->insurance_policy_pdf_path) ? '' : 'hidden' }}">
                        Remove
                    </button>
                </div>
