<x-admin-layout title="Cities" :white-bg="true">
    <div class="mb-6">
        <!-- Breadcrumbs -->
        <nav class="flex items-center gap-1.5 text-slate-400 dark:text-slate-500 text-xs font-medium mb-1.5">
            <a href="{{ route('admin.cities.index') }}" class="hover:text-slate-600 dark:hover:text-slate-300">Cities</a>
        ...
        <div class="flex items-center justify-between">
            <h1 class="text-2xl font-bold tracking-tight text-slate-950 dark:text-white">Cities</h1>
            @can('city.create')
                <a href="{{ route('admin.cities.create') }}" class="inline-flex items-center justify-center px-4 py-2 bg-primary-600 hover:bg-primary-500 text-white text-sm font-semibold rounded-lg shadow-sm transition">
                    <span>New city</span>
                </a>
            @endcan
        </div>
    ...
    <x-datatable-card tableId="cities-table" searchPlaceholder="Search">
        <x-slot:filters>
            <div>
                <label class="block text-xs font-semibold text-slate-500 dark:text-slate-400 mb-1">Status</label>
                <select data-filter-column="1" data-filter-type="regex" class="form-select w-full border border-slate-200 dark:border-slate-800 rounded-xl bg-white dark:bg-slate-900 text-slate-900 dark:text-slate-100 focus:border-primary-500 focus:ring-primary-500/20 text-sm h-10 px-3">
        ...
        @forelse ($cities as $city)
            <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition duration-150">
                <td class="px-6 py-4.5 whitespace-nowrap">
                    <span class="font-semibold text-slate-900 dark:text-white text-sm">{{ $city->name }}</span>
                </td>
                ...
                <td class="px-6 py-4.5 whitespace-nowrap text-right">
                    <div class="relative inline-block text-left" data-kebab-container>
                        ...
                        <div class="absolute right-0 mt-1 w-44 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-lg p-1 hidden z-40" data-kebab-menu>
                            @can('city.edit')
                                <a href="{{ route('admin.cities.edit', $city) }}" class="flex items-center gap-2 px-3 py-2 text-sm text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 rounded-md no-underline">
        ...
        @empty
            <tr>
                <td class="text-center text-slate-400 py-12">No cities found.</td>
                <td></td>
                <td></td>
            </tr>
        @endforelse
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            document.addEventListener('click', (e) => {
                ...
                if (btn) {
                    ...
                    deleteCityForm.action = deleteUrl;
