<x-admin-layout title="Agents" :white-bg="true">
    <div class="mb-6">
        <!-- Breadcrumbs -->
        ...
        <div class="flex items-center justify-between">
            <h1 class="text-2xl font-bold tracking-tight text-slate-950 dark:text-white">Agents</h1>
            @can('agent.create')
                <a href="{{ route('admin.agents.create') }}" class="inline-flex items-center justify-center px-4 py-2 bg-primary-600 hover:bg-primary-500 text-white text-sm font-semibold rounded-lg shadow-sm transition">
                    <span>New agent</span>
                </a>
            @endcan
        </div>
    ...
    <x-datatable-card tableId="agents-table" searchPlaceholder="Search">
        <x-slot:filters>
            <div>
        ...
        @forelse ($agents as $agent)
            <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition duration-150">
                <td class="px-6 py-4.5 whitespace-nowrap">
                    <div class="flex items-center gap-3">
                        @if ($agent->profilePhotoUrl())
                            <img src="{{ $agent->profilePhotoUrl() }}" alt="" class="w-9 h-9 rounded-full object-cover shrink-0">
                        @else
                            @php
                                $initials = '';
                        ...
                        @endif
                        <span class="font-semibold text-slate-900 dark:text-white">{{ $agent->name }}</span>
                    </div>
                </td>
                ...
                <td class="px-6 py-4.5 whitespace-nowrap text-sm text-slate-500 dark:text-slate-400" data-col="created" data-order="{{ $agent->created_at?->timestamp ?? 0 }}">{{ $agent->created_at?->format('M d, Y') ?? '—' }}</td>
                <td class="px-6 py-4.5 whitespace-nowrap text-right">
                    <div class="relative inline-block text-left" data-kebab-container>
                        ...
                        <div class="absolute right-0 mt-1 w-44 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-lg p-1 hidden z-40" data-kebab-menu>
                            <a href="{{ route('admin.agents.show', $agent) }}" class="flex items-center gap-2 px-3 py-2 text-sm text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 rounded-md no-underline">
                                <svg class="w-4 h-4 text-slate-400 shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                <span>View profile</span>
                            </a>

                            @can('agent.edit')
                                <a href="{{ route('admin.agents.edit', $agent) }}" class="flex items-center gap-2 px-3 py-2 text-sm text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 rounded-md no-underline">
                            ...
                            @can('agent.activate')
                                <form method="POST" action="{{ route('admin.agents.toggle-status', $agent) }}" class="block">
                                    @csrf
                                    @method('PATCH')
                                    <button type="submit" class="w-full flex items-center gap-2 px-3 py-2 text-sm {{ $agent->isActive() ? 'text-red-600 hover:bg-red-50 dark:hover:bg-red-950/20' : 'text-emerald-600 hover:bg-emerald-50 dark:hover:bg-emerald-950/20' }} rounded-md text-left">
                                        @if ($agent->isActive())
        ...
        document.addEventListener('DOMContentLoaded', () => {
            ...
            document.addEventListener('click', (e) => {
                ...
                if (btn) {
                    ...
                    deleteAgentForm.action = deleteUrl;
