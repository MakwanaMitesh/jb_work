<x-admin-layout title="Banks" :white-bg="true">
    <div class="mb-6">
        ...
        <div class="flex items-center justify-between">
            <h1 class="text-2xl font-bold tracking-tight text-slate-950 dark:text-white">Banks Master</h1>
            <a href="{{ route('admin.banks.create') }}" class="inline-flex items-center justify-center px-4 py-2 bg-primary-600 hover:bg-primary-500 text-white text-sm font-semibold rounded-lg shadow-sm transition">
                <span>New Bank</span>
            </a>
        </div>
    </div>

    <x-datatable-card tableId="banks-table" searchPlaceholder="Search Bank Name">
        <x-slot:filters>
            <div>
        ...
        <x-slot:thead>
            <th class="px-6 py-3.5 text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">Bank Name</th>
                ...
                <td class="px-6 py-4.5 whitespace-nowrap text-right">
                    <div class="relative inline-block text-left" data-kebab-container>
                        ...
                        <div data-kebab-menu class="hidden absolute right-0 mt-2 w-48 bg-white dark:bg-slate-900 rounded-xl shadow-lg border border-slate-200 dark:border-slate-800 py-1 z-50">
                            <a href="{{ route('admin.banks.edit', $bank) }}" class="flex items-center gap-2 px-4 py-2 text-sm text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800">
                            ...
                            <form method="POST" action="{{ route('admin.banks.toggle-status', $bank) }}">
                                @csrf
                                @method('PATCH')
                            ...
                            <form method="POST" action="{{ route('admin.banks.destroy', $bank) }}" onsubmit="return confirm('Are you sure you want to delete this Bank?')">
                                @csrf
                                @method('DELETE')
