<x-admin-layout title="Customer Constitutions" :white-bg="true">
    <div class="mb-6">
        ...
        <div class="flex items-center justify-between">
            <h1 class="text-2xl font-bold tracking-tight text-slate-950 dark:text-white">Customer Constitutions</h1>
            @can('constitutions.create')
                <a href="{{ route('admin.constitutions.create') }}" class="inline-flex items-center justify-center px-4 py-2 bg-primary-600 hover:bg-primary-500 text-white text-sm font-semibold rounded-lg shadow-sm transition">
    ...
    <x-datatable-card tableId="constitutions-table" searchPlaceholder="Search">
        <x-slot:filters>
            <div>
        ...
        @forelse ($constitutions as $constitution)
            <tr class="hover:bg-slate-50/50 dark:hover:bg-slate-800/30 transition duration-150">
                <td class="px-6 py-4.5 whitespace-nowrap">
                    <a href="{{ route('admin.constitutions.show', $constitution) }}" class="font-semibold text-primary-600 hover:underline text-sm">{{ $constitution->name }}</a>
                </td>
                <td class="px-6 py-4.5 whitespace-nowrap">
                    <span class="text-sm font-mono text-slate-650 dark:text-slate-350">{{ $constitution->code }}</span>
                </td>
                ...
                <td class="px-6 py-4.5 whitespace-nowrap">
                    <span class="text-sm text-slate-500">{{ $constitution->sort_order }}</span>
                </td>
                    ...
                    <div class="relative inline-block text-left" data-kebab-container>
                        ...
                        <div class="absolute right-0 mt-1 w-44 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-lg shadow-lg p-1 hidden z-40" data-kebab-menu>
                            @can('constitutions.edit')
                                <a href="{{ route('admin.constitutions.edit', $constitution) }}" class="flex items-center gap-2 px-3 py-2 text-sm text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 rounded-md no-underline">
                                ...
                                <form method="POST" action="{{ route('admin.constitutions.toggle-status', $constitution) }}" class="inline">
                                    @csrf
                                    @method('PATCH')
        ...
        @empty
            <tr>
                <td colspan="5" class="px-6 py-10 text-center text-sm text-slate-450">No customer constitutions found.</td>
            </tr>
        @endforelse
    </x-datatable-card>
