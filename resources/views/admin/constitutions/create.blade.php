<x-admin-layout title="Create Customer Constitution" :white-bg="true">
    <div class="mb-6">
        <nav class="flex items-center gap-1.5 text-slate-400 dark:text-slate-500 text-xs font-medium mb-1.5">
            ...
            <span class="text-slate-900 dark:text-slate-200">New</span>
        </nav>
        <h1 class="text-2xl font-bold tracking-tight text-slate-950 dark:text-white">New Customer Constitution</h1>
    </div>

    <form method="POST" action="{{ route('admin.constitutions.store') }}">
        @csrf
